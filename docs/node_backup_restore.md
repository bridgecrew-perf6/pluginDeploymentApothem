...
####   - Database files
Using the inbuilt postgres database utility, we take a full backup of the plugin node database **"plugin_apothem_db"**, which produces a single sql data file.
...
In the scenario where you are backing up any files for the fisrt time, we need to run the setup script to ensure that all the backup folder and permissions are in place.
  ...
  2. This will produce output to the terminal as it executes, the following is an example of what you can expect;
            ...
            nmadmin@plitest:~/pluginDeploymentApothem$ ./_plinode_setup_bkup.sh
            [sudo] password for nmadmin:
            COMPELTED BACKUP SETUP SCRIPT
            nmadmin@plitest:~/pluginDeploymentApothem$
...
Run the following commands to perform a **FULL** backup. As per the usage above this backups up both the conf files & the db.
    ...
    cd ~/pluginDeploymentApothem && ./_plinode_backup.sh -full
...
Run the following commands to perform a **CONFIG files** only backup
    ...
    cd ~/pluginDeploymentApothem && ./_plinode_backup.sh -conf
...
Run the following commands to perform a **DATABASE** only backup
    ...
    cd ~/pluginDeploymentApothem && ./_plinode_backup.sh -db
...
You should see something similar to the following;
    ...
    nmadmin@plitest:~/pluginDeploymentApothem$ ll /plinode_backups/
    total 628
    ...
    -rw-r--r--  1 nmadmin nodebackup 28277 Apr 12 22:43 plitest_plugin_apothem_db_2022_04_12_22_43.sql.gz.gpg
    -rw-r--r--  1 nmadmin nodebackup 28763 Apr 12 22:54 plitest_plugin_apothem_db_2022_04_12_22_54.sql.gz.gpg
    -rw-r--r--  1 nmadmin nodebackup 29059 Apr 15 22:52 plitest_plugin_apothem_db_2022_04_15_22_52.sql.gz.gpg
    nmadmin@plitest:~/pluginDeploymentApothem$
...
This is not a very involved operation with minimal steps as follows;
  ...
  2. Now to selecting the type & date-time stamp backup file to restore. You should be presented with a list of files similar to the following;
     **NOTE ::** _The list of files that you see will be dependent on how many backups you have performed._
            ...
            1) /plinode_backups/plitest_conf_vars_2022_04_12_22_43.tar.gz.gpg	       6) /plinode_backups/plitest_plugin_apothem_db_2022_04_13_08_25.sql.gz.gpg
            2) /plinode_backups/plitest_conf_vars_2022_04_13_10_09.tar.gz.gpg	       7) /plinode_backups/plitest_plugin_apothem_db_2022_04_13_08_29.sql.gz.gpg
            3) /plinode_backups/plitest_plugin_apothem_db_2022_04_12_22_43.sql.gz.gpg  8) /plinode_backups/plitest_plugin_apothem_db_2022_04_13_10_05.sql.gz.gpg
            4) /plinode_backups/plitest_plugin_apothem_db_2022_04_12_22_54.sql.gz.gpg  9) QUIT
            5) /plinode_backups/plitest_plugin_apothem_db_2022_04_13_08_21.sql.gz.gpg
            #?
...
#### Restore the conf files
  ...
  2. Lets kick off the "conf" files restore by running the main restore script;
            ...
            cd ~/pluginDeploymentApothem && ./_plinode_restore.sh
...
#### Perform a fresh node deployment install
  ...
  Now we can perform a fresh node installation which will re-use those existing credentials & settings
            ...
            cd ~/pluginDeploymentApothem && ./pli_node_scripts.sh fullnode
...
#### Restore the database
  ...
  **_NOTE ::_** **_The code detects the file selection and calls the appropriate function to handle the file._**
  ...
  3. Choose a "db" file you will then be presented with the scenario check message as follows; where you confirm which approach you wish to execute;
               ...
               RESTORE MENU - Restoring file: /plinode_backups/plitest_plugin_apothem_db_2022_04_13_10_05.sql.gz.gpg
                   ######################################################################################
                   ######################################################################################
                   ##
                   ##      RESTORE SCENARIO CONFIRMATION...
  ...
  5. Having confirmed Yes to the scenario confirmation message, this sets a flag within the code the forces a rebuild of the External Initiator (EI) process. We see the script restore messages as follows;
               ...
               DB RESTORE.... unzip file name: /plinode_backups/plitest_plugin_apothem_db_2022_04_13_10_05.sql.gz
               DB RESTORE.... psql file name: /plinode_backups/plitest_plugin_apothem_db_2022_04_13_10_05.sql
  ...
  8. The following messages immediately follow, confirming the rebuild of the External Initiator (EI) & restarting the PM2 process
            ...
            [PM2] Starting /home/nmadmin/pluginDeploymentApothem/3_initiatorStartPM2.sh in fork_mode (1 instance)
            [PM2] Done.
            ┌─────┬────────────────────────┬─────────────┬─────────┬─────────┬──────────┬────────┬──────┬───────────┬──────────┬──────────┬──────────┬──────────┐
            │ id  │ name                   │ namespace   │ version │ mode    │ pid      │ uptime │ ↺    │ status    │ cpu      │ mem      │ user     │ watching │
            ├─────┼────────────────────────┼─────────────┼─────────┼─────────┼──────────┼────────┼──────┼───────────┼──────────┼──────────┼──────────┼──────────┤
  ...
  15. This command with print the local Node Address to the terminal screen for you to check.
            ...
            nmadmin@plitest:~/pluginDeploymentApothem$ ./pli_node_scripts.sh address
