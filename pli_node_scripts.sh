...
FUNC_NODE_DEPLOY(){
    ...
    #fi
    #cd /$PLI_BASE_DIR
    #git clone https://github.com/KumarPint00/pluginDeploymentApothem.git && cd pluginDeploymentApothem
...
echo $(sudo -u postgres -i psql -d plugin_apothem_db -t -c"select json from keys where id=1;")  > ~/"plinode_$(hostname -f)_keys_${FDATE}".json
...
FUNC_INITIATOR(){
    ...
    cat <<EOF > $BASH_FILE3
#!/bin/bash
export EI_DATABASEURL=postgresql://postgres:${DB_PWD_NEW}@127.0.0.1:5432/plugin_apothem_db?sslmode=disable
...
external-initiator "{\"name\":\"$PLI_E_INIT_NAME\",\"type\":\"xinfin\",\"url\":\"https://rpc.apothem.network\"}" --chainlinkurl "http://localhost:6688/"
EOF
    #sleep 1s
    #cat $BASH_FILE3
